...
local RoundFloat = function(number, num)
    return math.floor(number*(10^num)+0.5) / (10^num)
end
...
local GetEntityInfo = function(entity)
    ...
    local entityData    = {
        ...
        'Script owner: ~y~'..(GetEntityScript(entity) or 'None'),
        ' '
    }
    ...
    if entityType == 1 then
        ...
        table.insert(entityData, 'Relationship Group: ~y~'.. (Entities[pedRelationshipGroup] or 'Custom Relationship'))
        table.insert(entityData, 'Relationship to Player: ~y~'..GetPedRelationshipType(GetRelationshipBetweenPeds(entity, PlayerPedId())))
    elseif entityType == 2 then
        table.insert(entityData, 'RPM: ~y~'..RoundFloat(GetVehicleCurrentRpm(entity), 2))
        table.insert(entityData, (useKph and 'KPM:' or 'MPH:')..' ~y~'..RoundFloat((GetEntitySpeed(entity)*(useKph and 3.6 or 2.23694)), 0))
            ...
            if EntityFreeAim then
                ...
                if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                    ...
                    if IsControlJustReleased(0, 47) then -- Freeze entities
                        ...
                        NewLog((FrozenEntities[entity] and 'success' or 'error'), 'You have '..(FrozenEntities[entity] and 'Frozen' or 'Unfrozen')..' the freeaim entity')
                    end
                    ...
                    if IsControlJustReleased(0, 38) then -- Delete entity
                        ...
                        if not DoesEntityExist(entity) then
                            NewLog('success', 'Entity deleted')
                        else
                            NewLog('error', 'Error deleting entity')
                        end
                    end
                else
                    FreeAimEntity = nil
